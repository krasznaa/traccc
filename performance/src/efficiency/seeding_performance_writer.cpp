...
void seeding_performance_writer::write(
    ...
    for (edm::seed_collection::const_device::size_type i = 0u; i < seeds.size();
        ...
        std::array<measurement, 3> seed_measurements{
            measurements.at(
                spacepoints.at(sd.bottom_index()).measurement_indices().at(0)),
            measurements.at(
                spacepoints.at(sd.middle_index()).measurement_indices().at(0)),
            measurements.at(
                spacepoints.at(sd.top_index()).measurement_indices().at(0))};
