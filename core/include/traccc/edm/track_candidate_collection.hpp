...
class track_candidate : public BASE {
    ...
    ///
    TRACCC_HOST_DEVICE
    auto& nholes() { return BASE::template get<3>(); }
    /// The number of holes in the track pattern (const)
    ///
    /// @return A (const) vector of unsigned integers
    ///
    TRACCC_HOST_DEVICE
    const auto& nholes() const { return BASE::template get<3>(); }
    ...
    ///
    TRACCC_HOST_DEVICE
    auto& measurement_indices() { return BASE::template get<4>(); }
    /// The indices of the measurements associated to the track (const)
    ///
    /// @return A (const) jagged vector of unsigned integers
    ///
    TRACCC_HOST_DEVICE
    const auto& measurement_indices() const { return BASE::template get<4>(); }
