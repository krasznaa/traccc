...
auto combinatorial_kalman_filter_algorithm::operator()(
    ...
    for (unsigned int step = 0;
        ...
        apply_interaction_kernel(
            {n_in_params, det, in_params_buffer, param_liveness_buffer});
...

        {
            ...
            // Launch the kernel.
            find_tracks_kernel({n_in_params,
                                det,
                                measurements_view,
                                in_params_buffer,
        ...
        if ((n_candidates > 0) &&
            ...
            fill_finding_duplicate_removal_sort_keys_kernel(
                {links_buffer, param_liveness_buffer,
                 link_last_measurement_buffer, param_ids_buffer, n_candidates,
                 step_to_link_idx_map[step], n_measurements});
            ...
            remove_duplicates_kernel(
                {links_buffer, link_last_measurement_buffer, param_ids_buffer,
                 param_liveness_buffer, n_candidates,
                 step_to_link_idx_map[step], n_measurements, step});
        }
        ...
        if (n_candidates > 0) {
            ...
            propagate_to_next_surface_kernel(
                {det, bfield, in_params_buffer, param_liveness_buffer,
                 param_ids_buffer, links_buffer, step_to_link_idx_map[step],
                 step, tips_buffer, tip_length_buffer, tmp_jacobian_buffer});
        }
    ...
    if (n_tips_total > 0 &&
        ...
        if (mr().host) {
            vecmem::async_size size =
                copy().get_size(tip_length_buffer, *(mr().host));
            // Here we could give control back to the caller, once our code
            // allows for it. (coroutines...)
            n_tips_total = size.get();
        } else {
            n_tips_total = copy().get_size(tip_length_buffer);
        }
    }
